...
const Dropdown = ({value, onChangeHandler}: DropdownProps) => {
  ...
  return (
        ...
        <SelectContent>
            {categories.length > 0 && categories.map((category) => <SelectItem className='select-item p-regular-14' key={category._id} value={category._id}>{category.name}</SelectItem>)}
            <AlertDialog >
